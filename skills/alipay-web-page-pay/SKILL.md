...
Do not assume the implementing agent can complete Alipay platform console operations automatically.
...
## Human Operations Output Rule (Mandatory)

Whenever this skill is used for sandbox/production setup guidance, include:

1. exact Alipay console links
2. click-path style operation steps
3. deliverables user must send back to the implementing agent
4. clear separation: `human does` vs `agent does`

Use this reference as the primary operator script:

- [references/alipay-platform-operator-playbook.md](references/alipay-platform-operator-playbook.md)
...
8. Human preflight checklist with direct links and step-by-step operator actions.
...
Implementing agent should continue implementation with mock mode fallback when sandbox account setup is pending.
...
- Alipay console links and human operator playbook: [references/alipay-platform-operator-playbook.md](references/alipay-platform-operator-playbook.md)
